...
Usage:
    python acquisition_optimized.py
    
Connect to device → Verify channel mapping → Start acquisition
Data streams via LSL and saves to JSON session files
"""
...
class PacketParser:
    ...
    def parse(self, packet_bytes: bytes) -> Packet:
        ...
        # Convert ADC to µV
        ch0_uv = SignalProcessing.adc_to_uv(ch0_raw)
        ch1_uv = SignalProcessing.adc_to_uv(ch1_raw)
...
class UnifiedAcquisitionApp:
    ...
    def setup_ui(self):
        ...
        # Channel mapping
        mapping_frame = ttk.LabelFrame(left_frame, text="🎛️ Channel Mapping", padding=10)
        ...
        # Recording
        rec_frame = ttk.LabelFrame(left_frame, text="🎙️ Recording", padding=10)
        rec_frame.pack(fill="x", pady=5)
        self.rec_btn = ttk.Button(rec_frame, text="⏺️ Start Recording", 
        ...
        self.fig = Figure(figsize=(13, 6), dpi=100, facecolor='white')
        self.fig.patch.set_facecolor('#f0f0f0')
        
        self.ax_ch0 = self.fig.add_subplot(211)
        self.line_ch0, = self.ax_ch0.plot(self.x_axis, self.vis_ch0, linewidth=1.2, label='Ch0 (Raw)', color='#1f77b4')
        ...
        self.line_ch1, = self.ax_ch1.plot(self.x_axis, self.vis_ch1, linewidth=1.2, label='Ch1 (Raw)', color='#ff7f0e')
    ...
    def start_acquisition(self):
        ...
        try:
            ...
            self.rec_btn.config(state="normal", text="⏹️ Stop Recording")
    ...
    def stop_acquisition(self):
        ...
        self.start_btn.config(state="normal")
        self.stop_btn.config(state="disabled")
        self.rec_btn.config(state="normal", text="⏺️ Start Recording")
        self.status_label.config(text="✅ Connected", foreground="green")
        print("⏹️ Acquisition stopped")
    ...
    def toggle_recording(self):
        ...
        if self.is_recording:
            self.rec_btn.config(text="⏹️ Stop Recording")
            print("🔴 Recording ON")
        else:
            self.rec_btn.config(text="⏺️ Start Recording")
            print("⚪ Recording OFF")
    ...
    def _update_ui(self):
        ...
        if self.show_graph_var.get():
            try:
                ...
                # Auto-scale Y (optional, or keep fixed/manual)
                # For sweep, usually good to fix, but we can auto-scale
                # based on recent data or full buffer
                if self.packet_count % 10 == 0:  # Don't rescale every frame
                     self.ax_ch0.set_ylim(np.min(self.vis_ch0)-100, np.max(self.vis_ch0)+100)
                     self.ax_ch1.set_ylim(np.min(self.vis_ch1)-100, np.max(self.vis_ch1)+100)
