...
def create_branch_from_base(repo, branch_name, base_branch, existing_branches):
    ...
    if base_branch not in existing_branches:
        print(f"   ⚠️  Base branch '{base_branch}' doesn't exist yet, will create it first")
        return False
    ...
    print(f"   🔀 Checking out base: {base_branch}")
...
def main():
    ...
    develop
      ↑
      ├─ sensors/
      │   ├─ feature/eeg-bandpass
      │   ├─ feature/eog-blink-detection
      │   └─ feature/emg-envelope
      │
      ├─ backend/
      │   ├─ feature/websocket-server
      │   ├─ feature/fusion-server
      │   └─ feature/ml-training
      │
      ├─ frontend/
      │   ├─ feature/live-charts
      │   ├─ feature/command-visualizer
